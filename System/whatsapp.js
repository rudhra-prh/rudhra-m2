...
exports.serialize = (Rudhra, m, options = {}) => {
  if (!m) return m;
  ...
  if (m.key) {
    ...
    m.sender = jidNormalizedUser(
      (m.fromMe && Rudhra.user?.id) || m.key.participant || m.from || ""
    );
  }
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe =
        m.quoted.sender == jidNormalizedUser(Rudhra.user && Rudhra.user?.id);
      m.quoted.text =
        m.quoted.msg?.text ||
      ...
      m.quoted.delete = () =>
        Rudhra.sendMessage(m.quoted.from, { delete: vM.key });
      m.quoted.download = (pathFile) =>
        Rudhra.downloadMediaMessage(m.quoted.msg, pathFile);
    }
  }
  m.download = (pathFile) => Rudhra.downloadMediaMessage(m.msg, pathFile);
  m.body = m.text =
  ...
  m.reply = (text, chatId = m.from, options = {}) =>
    Buffer.isBuffer(text)
      ? Rudhra.sendFile(chatId, text, "file", "", m, { ...options })
      : Rudhra.sendText(chatId, text, m, { ...options });
